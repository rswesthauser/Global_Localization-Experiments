...
There are configurations that have an error bigger than 100 meters on some or all pictures/positions.
```{r}
library(ggplot2)
library(ggpubr)
library(gridExtra )
...
Plotting the mean error on each image of the drone flight.
The X axis are the image taken on each moment, while the Y axis represemnt the mean error in meters, of the estimation algorithm.
